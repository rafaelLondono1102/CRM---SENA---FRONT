...
const CrearOportunidadesComponent = ({ oportunidadEdited, setIsEditing }) => {
    const router = useRouter();
    const [oportunidad, setOportunidad] = useState({
        _id: oportunidadEdited._id,
        priority: oportunidadEdited.priority,
        projectName: oportunidadEdited.projectName,
        growthPotential: oportunidadEdited.growthPotential,
        articulationArea: oportunidadEdited.articulationArea,
        articulationInteres: oportunidadEdited.articulationInteres,
        initialTRL: oportunidadEdited.initialTRL,
        achivedTRL: oportunidadEdited.achivedTRL,
        deliver: oportunidadEdited.deliver,
        abandonReason: oportunidadEdited.abandonReason,
        prototipeUse: oportunidadEdited.prototipeUse,
        inovationLevel: oportunidadEdited.inovationLevel,
        observations: oportunidadEdited.observations
    });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const result = await axios.put(`http://localhost:9005/api/v1/oportunity/${oportunidad._id}`, oportunidad);
            console.log(result)
            alert("Se ha editado satisfactoriamente una Oportunidad");
